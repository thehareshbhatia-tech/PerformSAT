...
const newBlock = `  const renderDiagnosticView = () => {
    ...
    const renderBlock = (block, idx) => {
              ...
              {block.items.map((s, i) => {
                ...
                return (
                  <div key={i} className="stat-card">
                    ...
                    <div style={{ fontFamily: 'var(--font-ui)', fontSize: '12px', fontWeight: '700', color: colors.text.secondary, marginTop: '12px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{s.label}</div>
                    {s.subtext && <div style={{ fontFamily: 'var(--font-ui)', fontSize: '13px', color: colors.text.muted, marginTop: '6px', fontWeight: '500' }}>{s.subtext}</div>}
                  </div>
                );
              })}
            </div>
      ...
      if (block.id === 'primaryCause') {
        return (
          <div key={block.id} style={{ padding: '36px' }}>
            <h3 style={{ fontFamily: 'var(--font-ui)', fontSize: '20px', fontWeight: '700', color: colors.text.primary, marginBottom: '24px', letterSpacing: '-0.02em' }}>Core Issues</h3>
            {block.transition && (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {block.items.map((d, i) => {
                if (d.type === 'pattern') {
                  ...
                  return (
                    <div key={i} style={{ padding: '24px', borderRadius: '20px', background: 'rgba(255,255,255,0.9)', border: \`1px solid \${sc.border}\`, position: 'relative', overflow: 'hidden', boxShadow: '0 4px 16px rgba(0,0,0,0.02)' }}>
                      <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: '4px', background: sc.dot }} />
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
                        <span style={{ fontFamily: 'var(--font-ui)', flex: 1, fontSize: '16px', fontWeight: '600', color: colors.text.primary, lineHeight: '1.5' }}>{d.text}</span>
                        {d.estimatedPointGain && (
                          <span style={{ fontFamily: 'var(--font-ui)', fontSize: '13px', fontWeight: '800', color: 'var(--color-success-600)', background: 'var(--color-success-100)', padding: '6px 12px', borderRadius: '10px', whiteSpace: 'nowrap' }}>+{d.estimatedPointGain} pts</span>
                        )}
                      </div>
                      {d.impact && (
                ...
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '8px 0' }}>
                    <span style={{ flexShrink: 0, width: '6px', height: '6px', borderRadius: '50%', background: 'var(--color-slate-300)', marginTop: '10px' }} />
                    <span style={{ fontFamily: 'var(--font-ui)', fontSize: '16px', color: colors.text.secondary, lineHeight: '1.6' }}>{d.text}</span>
                  </div>
                );
              })}
            </div>
      ...
      if (block.id === 'behaviorAmplifier') {
        return (
          <div key={block.id} style={{ padding: '36px' }}>
            <h3 style={{ fontFamily: 'var(--font-ui)', fontSize: '20px', fontWeight: '700', color: colors.text.primary, marginBottom: '24px', letterSpacing: '-0.02em' }}>Test Behavior</h3>
            {block.transition && (
              <div style={{ fontFamily: 'var(--font-ui)', fontSize: '15px', color: colors.text.secondary, fontWeight: '500', marginBottom: '24px', lineHeight: '1.5' }}>{block.transition}</div>
            )}
            <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {block.items.map((pt, i) => (
                <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '14px', fontSize: '16px', color: colors.text.secondary, lineHeight: '1.6', fontFamily: 'var(--font-ui)' }}>
                  <span style={{ flexShrink: 0, width: '8px', height: '8px', borderRadius: '50%', background: 'var(--color-slate-400)', marginTop: '10px' }} />
                  {pt}
                </li>
              ))}
            </ul>
