...
export const mRadiansBlocks = [
  ...
  {
    type: 'text',
    content: "The symbol $\\pi$ denotes a number, approximately $3.14$, so $\\pi$ radians is slightly more than $3$ radians — and that measure is exactly a half-circle, $180°$. Doubling gives a full circle, $360° = 2\\pi$; halving gives a right angle, $90° = \\frac{\\pi}{2}$. Anchoring every conversion and every unit-circle value on this one equivalence removes the need to memorize anything further.",
  },

  { type: 'heading', content: 'Converting between degrees and radians' },
  {
    type: 'text',
    content: "Rather than memorizing whether to multiply or divide by $\\frac{\\pi}{180}$ — a rule easily reversed under time pressure — choose the fraction that **cancels the unit being left behind**. To convert to radians, multiply by $\\frac{\\pi}{180}$ so the degrees cancel; to convert to degrees, multiply by $\\frac{180}{\\pi}$ so the radians cancel. The units themselves determine the correct factor.",
  },
  {
    type: 'keyInsight',
    content: 'Check the magnitude of the result. $45°$ should convert to a small value — $\\frac{\\pi}{4}\\approx 0.79$ — not a large one. A result far too large indicates the conversion fraction was inverted.',
  },

  { type: 'heading', content: 'Arc length and sector area' },
  {
    type: 'text',
    content: "When the angle $\\theta$ is **in radians**, arc length is $s = r\\theta$ — radius times angle, with no conversion required. That simplicity is the reason radians exist: a radian angle measures its arc directly in radius-lengths. A sector occupies that same fraction of the circle, so its area is $A = \\tfrac{1}{2}r^2\\theta$. The standard error is substituting a *degree* angle directly into $r\\theta$. If the angle is given in degrees, convert it to radians first — or take the fraction $\\frac{\\theta}{360}$ of the whole circle instead.",
  },
  {
    type: 'tip',
    content: 'If a problem gives the angle in radians and asks for arc length, compute $r\\theta$ directly. Converting to degrees first adds an unnecessary step and an additional opportunity for error.',
  },

  { type: 'heading', content: 'The unit circle: $(\\cos\\theta,\\ \\sin\\theta)$' },
  {
    type: 'text',
    content: "The unit circle has radius $1$ and is centered at the origin. A point on it at angle $\\theta$, measured from the positive $x$-axis, has coordinates $(\\cos\\theta,\\ \\sin\\theta)$: the $x$-coordinate **is** the cosine and the $y$-coordinate **is** the sine. These values are read directly from the point's position rather than computed.",
  },
  {
    type: 'table',
  ...
  { type: 'heading', content: 'Determining the sign from the quadrant' },
  {
    type: 'text',
  ...
  { type: 'heading', content: 'Worked examples' },
  {
    ...
    steps: [
      { label: 'Choose the factor', content: "The conversion leaves degrees, so multiply by $\\frac{\\pi}{180}$ so that the degrees cancel." },
