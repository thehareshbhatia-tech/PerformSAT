...
const BlockRenderers = {
  ...
  table: (block, idx) => (
    <div key={idx} className="block-table-container">
      {block.title && <div className="block-label">{renderRichText(block.title)}</div>}
    ...
    <div key={idx} className="block-grid-auto">
      {block.items.map((item, fi) => (
        <div key={fi} className="block-math block-math--grid-item">
          <div className="block-label">{renderRichText(item.label)}</div>
          <div className="block-math-formula" style={{ fontSize: '1.25rem' }}>
            <MathText>{item.formula}</MathText>
          </div>
          {item.note && <div className="block-math-note">{renderRichText(item.note)}</div>}
        </div>
      ))}
    </div>
  ),
    ...
    return (
      <div key={idx} className={`block-emphasis ${variantClass}`}>
        ...
        <div className="block-emphasis-content">
          {block.title && <div className="block-emphasis-title">{renderRichText(block.title)}</div>}
  ...
  trapCard: (block, idx) => (
    <div key={idx} className="block-emphasis block-emphasis--error">
      ...
      <div className="block-emphasis-content">
        <div className="block-emphasis-title">{renderRichText(block.title || 'Common Trap')}</div>
  ...
  strategyCard: (block, idx) => (
    <DataCard key={idx} style={{ padding: '1.5rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem' }}>
        ...
        <div style={{ fontWeight: 600, color: 'var(--color-slate-900)' }}>{renderRichText(block.title)}</div>
        {block.timing && (
          <span className="block-label" style={{ margin: '0 0 0 auto' }}>{renderRichText(block.timing)}</span>
        )}
      </div>
          ...
          <div className="block-worked-steps">
            {block.steps.map((step, si) => (
              <div key={si} className={`block-worked-step ${si === block.steps.length - 1 ? 'block-worked-step--final' : ''}`}>
                ...
                <div className="block-worked-step-content">
                  {step.label && <div className="block-label">{renderRichText(step.label)}</div>}
  ...
  steps: (block, idx) => (
    <div key={idx}>
      {block.title && <div className="block-heading">{renderRichText(block.title)}</div>}
      <div className="block-worked-steps">
    ...
    <div key={idx} className="block-grid-auto">
      {block.items.map((item, ii) => (
        <div key={ii} className="block-worked-step" style={{ flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
          ...
          <div style={{ fontWeight: 600, color: 'var(--color-slate-900)', fontSize: '0.9375rem' }}>{renderRichText(item.label)}</div>
          <div style={{ color: 'var(--color-slate-500)', fontSize: '0.8125rem' }}>{renderRichText(item.description)}</div>
        </div>
      ))}
    </div>
  ),
