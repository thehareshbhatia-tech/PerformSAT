/**
 * PaywallScreen — the SEVA Premium subscribe wall (spec 2026-07-01).
 *
 * Mounted at view='paywall'; every locked action routes here. Shows the two
 * plans ($50/mo, $250/yr), the caller's trial state, and redirects to hosted
 * Stripe Checkout — no card form ever renders in-app. Expired students can
 * still go back to the dashboard (read-only surfaces stay open).
 */
import React, { useState } from 'react';
import { startCheckout } from '../../services/billingService';
import './PaywallScreen.css';

const FEATURES = [
  'All 12 full-length adaptive practice tests',
  'AI tutor on every question — explanations that actually teach',
  'Unlimited drills, review queue, and pacing practice',
  'Personalized study plan that updates with every test',
];

function CheckIcon() {
  return (
    <svg className="pw-check" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      <polyline points="20 6 9 17 4 12" />
    </svg>
  );
}

function statusLine(entitlement) {
  if (!entitlement) return null;
  if (entitlement.phase === 'trial') {
    const d = entitlement.trialDaysLeft;
    return { cls: 'is-trial', text: `Free trial: ${d} day${d === 1 ? '' : 's'} left` };
  }
  if (entitlement.phase === 'expired') {
    return { cls: 'is-expired', text: 'Your free trial has ended' };
  }
  if (entitlement.phase === 'grace') {
    return { cls: 'is-grace', text: 'Payment issue — update your card to keep access' };
  }
  return null;
}

function PaywallScreen({ entitlement, onBack }) {
  const [redirecting, setRedirecting] = useState(null); // 'monthly' | 'annual' | null
  const [error, setError] = useState(null);

  const subscribe = async (plan) => {
    if (redirecting) return;
    setError(null);
    setRedirecting(plan);
    try {
      await startCheckout(plan);
      // Browser navigates away on success; nothing to reset.
    } catch (err) {
      setError(err.message || 'Could not start checkout. Please try again.');
      setRedirecting(null);
    }
  };

  const status = statusLine(entitlement);
  const expired = entitlement?.phase === 'expired';

  return (
    <div className="pw-shell">
      <div className="pw-column">
        <button type="button" className="pw-back" onClick={onBack}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            <path d="m15 18-6-6 6-6" />
          </svg>
          Back to dashboard
        </button>

        {status && <div className={`pw-status ${status.cls}`}>{status.text}</div>}

        <h1 className="pw-title">
          {expired ? 'Keep your prep going' : 'Go Premium'}
        </h1>
        <p className="pw-sub">
          {expired
            ? 'Your scores and progress are saved. Subscribe to pick up exactly where you left off — every test, drill, and tutor session unlocks instantly.'
            : 'One plan, everything included. Subscribe now and nothing changes when your trial ends.'}
        </p>

        {error && <p className="pw-error" role="alert">{error}</p>}

        <div className="pw-plans">
          <div className="pw-plan">
            <h2 className="pw-plan-name">Monthly</h2>
            <div className="pw-plan-price">
              <span className="pw-plan-amount">$50</span>
              <span className="pw-plan-cadence">/ month</span>
            </div>
            <p className="pw-plan-note">Cancel anytime</p>
            <button
              type="button"
              className="pw-plan-cta is-secondary"
              onClick={() => subscribe('monthly')}
              disabled={!!redirecting}
            >
              {redirecting === 'monthly' ? 'Opening checkout…' : 'Start Monthly'}
            </button>
            <ul className="pw-features">
              {FEATURES.map((f) => (
                <li key={f}><CheckIcon />{f}</li>
              ))}
            </ul>
          </div>

          <div className="pw-plan is-featured">
            <span className="pw-plan-flag">Best value</span>
            <h2 className="pw-plan-name">Annual</h2>
            <div className="pw-plan-price">
              <span className="pw-plan-amount">$250</span>
              <span className="pw-plan-cadence">/ year</span>
            </div>
            <p className="pw-plan-note">Save $350 vs monthly</p>
            <button
              type="button"
              className="pw-plan-cta is-primary"
              onClick={() => subscribe('annual')}
              disabled={!!redirecting}
            >
              {redirecting === 'annual' ? 'Opening checkout…' : 'Start Annual'}
            </button>
            <ul className="pw-features">
              {FEATURES.map((f) => (
                <li key={f}><CheckIcon />{f}</li>
              ))}
            </ul>
          </div>
        </div>

        <p className="pw-fineprint">
          Secure checkout by Stripe. Promo codes are accepted on the payment page.
          Manage or cancel your plan anytime from Profile. By subscribing you agree
          to the <a href="/terms" target="_blank" rel="noreferrer">Terms</a> and{' '}
          <a href="/privacy" target="_blank" rel="noreferrer">Privacy Policy</a>.
        </p>
      </div>
    </div>
  );
}

export default PaywallScreen;
