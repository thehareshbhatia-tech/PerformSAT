...
// ── Type-first matching (preferType) — the Practice Bank drill-down ───────────

describe('pickSimilarQuestion — preferType (same question TYPE)', () => {
  it('matches the same TYPE first and skips the skill dispatcher entirely', () => {
    const typeMatch = makeQ({ id: 'type-match' });
    const skillMatch = makeQ({ id: 'skill-match' });
    const patternDisp = jest.fn(() => [typeMatch]);
    const skillDisp = jest.fn(() => [skillMatch]);
    const result = pickSimilarQuestion({
      currentQuestion: makeQ({ explanation: '**SAT Pattern: Evaluate Function**' }),
      preferType: true,
      mathPatternDispatcher: patternDisp,
      mathDispatcher: skillDisp,
      extractPattern: () => 'evaluate-function',
    });
    expect(result.kind).toBe('ok');
    expect(result.matchedBy).toBe('type');
    expect(result.typeSlug).toBe('evaluate-function');
    expect(result.question).toBe(typeMatch);
    expect(patternDisp).toHaveBeenCalledWith(['evaluate-function'], expect.any(Object));
    expect(skillDisp).not.toHaveBeenCalled();
  });

  it('falls back to the skill tier when the type pool is empty', () => {
    const skillMatch = makeQ({ id: 'skill-match' });
    const patternDisp = jest.fn(() => []); // no same-type items left
    const skillDisp = jest.fn(() => [skillMatch]);
    const result = pickSimilarQuestion({
      currentQuestion: makeQ(),
      preferType: true,
    ...
    expect(result.kind).toBe('ok');
    expect(result.matchedBy).toBe('skill');
    expect(result.question).toBe(skillMatch);
  });

  it('falls back to the skill tier when no type can be derived', () => {
    const skillMatch = makeQ({ id: 'skill-match' });
    const patternDisp = jest.fn(() => [makeQ({ id: 'should-not-be-used' })]);
    const skillDisp = jest.fn(() => [skillMatch]);
    const result = pickSimilarQuestion({
      currentQuestion: makeQ(),
      preferType: true,
      mathPatternDispatcher: patternDisp,
      mathDispatcher: skillDisp,
      extractPattern: () => null, // no SAT Pattern header on the item
    });
    expect(result.matchedBy).toBe('skill');
    expect(patternDisp).not.toHaveBeenCalled();
  });

  it('never consults the type dispatcher when preferType is false (default)', () => {
    const patternDisp = jest.fn(() => [makeQ({ id: 'type' })]);
    pickSimilarQuestion({
      currentQuestion: makeQ({ explanation: '**SAT Pattern: X**' }),
      mathDispatcher: makeMockDispatcher([makeQ({ id: 'skill' })]),
      mathPatternDispatcher: patternDisp,
      extractPattern: () => 'x',
    });
    expect(patternDisp).not.toHaveBeenCalled();
  ...
  it('routes R&W type matching through the R&W type extractor + dispatcher', () => {
    const typeMatch = makeQ({ id: 'rw-type-match', section: 'rw' });
    const rwPatternDisp = jest.fn(() => [typeMatch]);
    const deriveRWType = jest.fn(() => 'transitions');
    const result = pickSimilarQuestion({
      currentQuestion: makeQ({ id: 'rw-abc', section: 'rw' }),
      preferType: true,
      rwPatternDispatcher: rwPatternDisp,
      deriveRWType,
    });
    expect(result.kind).toBe('ok');
    expect(result.matchedBy).toBe('type');
    expect(deriveRWType).toHaveBeenCalledTimes(1);
    expect(rwPatternDisp).toHaveBeenCalledWith(['transitions'], expect.any(Object));
  });
});
