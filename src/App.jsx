...
const PerformSAT = () => {
  ...
  const [view, setView] = useState('dashboard'); // 'dashboard', 'modules', 'learn', 'practice', 'practiceTests', 'takingTest', 'profile', 'studyPlan', 'diagnosticReport', 'tutor'
  ...
  const renderContentBlocksOnly = () => {
    if (!currentLesson?.content?.blocks) return null;
    return currentLesson.content.blocks.map((block, idx) => {
          switch (block.type) {
            case 'text':
  ...
  return (
    ...
    <BrowserRouter>
      ...
      <main id="main-content" style={{
        maxWidth: view === 'learn' || view === 'takingTest' ? '100%' : (view === 'dashboard' || view === 'practiceTests' || view === 'diagnosticReport' || view === 'studyPlan') ? '960px' : '800px',
        margin: '0 auto',
        ...
        {view === 'learn' && activeModule && (
          <LearnWorkspace
            ...
            onBack={() => { setView('modules'); setActiveModule(null); setActiveLesson(null); }}
            onMarkComplete={markLessonComplete}
            isLessonCompleted={isLessonCompleted}
            moduleProgress={getModuleProgress(activeModule, currentModuleLessons)}
            renderLessonContent={renderContentBlocksOnly}
