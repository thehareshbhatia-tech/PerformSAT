...
6. Read one-variable equations as constant-coordinate lines: every point on y = c shares that y (horizontal); every point on x = c shares that x (vertical). Derive the slopes instead of memorizing them — between two points of a horizontal line Δy = 0, so the slope fraction has a zero NUMERATOR and equals 0; between two points of a vertical line Δx = 0, so the fraction has a zero DENOMINATOR and the slope is undefined. Zero on top and zero on the bottom are different failures, and that asymmetry is why y = c fits slope-intercept form while x = c cannot.
...
- **vertical-horizontal-swap** — believing x = 3 is a horizontal line "because the x-axis is horizontal," and pairing zero slope with the wrong one of the two. Forms because the two degenerate lines are memorized as an unanchored pair and the axis name pulls attention toward the wrong direction; the constant-coordinate reading never got installed. Produces mirrored picks on which-equation-is-this-line items and wrong slope claims (0 vs undefined) — choice sets offer both lines and both slope descriptions.
...
- vertical-horizontal-swap: picks x = c for a drawn horizontal line (or vice versa); calls a vertical line's slope 0 or a horizontal line's slope undefined; explain-back cites the axis name ("x = 3 goes along the x-axis").
...
- vertical-horizontal-swap: plot two points of x = 3 and read the shared coordinate aloud — the constant-coordinate meaning replaces the axis-name anchor. Then the slope derivation once: Δy = 0 on top gives 0; Δx = 0 on the bottom gives undefined. If the division-by-zero distinction itself is fuzzy, revisit [[slope-as-rate-of-change]].
...
## Difficulty ladder
- **Easy:** read m, b from clean slope-intercept; identify a form; match x = c / y = c to their lines and slopes (0 vs undefined).
