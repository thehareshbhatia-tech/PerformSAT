...
- **parity-sign-blindness** — assigning the wrong sign to a genuinely negative base raised to a power: declaring (−2)⁶ negative ("a negative stays negative") or (−2)⁵ positive ("powers make things positive"). Distinct from **bare-negative-base**: there the parenthesization/precedence is misread; here the base is unambiguously negative and the SIGN RULE itself is missing — the student never connected the exponent's parity to the pairing-off of negative factors. Forms because sign outcomes were memorized as a slogan in one direction or the other instead of derived by counting minuses. Produces sign-flip distractors on evaluations, and wrong must-be-true picks on items comparing (−x)ⁿ across even/odd n.
...
- **parity-sign-blindness**: sign errors that persist even when the base is parenthesized — distinguishes it from **bare-negative-base**; the direction of the error (always-negative vs always-positive) reveals which slogan they memorized. Misses on even/odd comparison items ((−x)⁴ vs (−x)³ must-be-true stems) with correct magnitude arithmetic.
...
- **parity-sign-blindness** → count the minuses: expand (−2)³ and (−2)⁴ into explicit factors and pair the negatives off — evens all cancel, odds leave one survivor. Then the rule stated from the derivation, not as a slogan: even power of a negative is positive, odd stays negative. One mixed-parity evaluation set confirms it landed; if the expansion itself confuses, reteach the repeated-multiplication definition first.
