...
/**
 * Resolve Intervention Loop — Component 5 of the Closed-Loop Data Intelligence System
 *
 * Triggered when progress/{userId} is updated (new practice test completed).
 * Connects cause (teaching) to effect (test performance) by:
 * 1. Resolving unresolved interventions with post-test metrics
 * 2. Updating student fingerprint
 * 3. Validating previous predictions
 * 4. Generating new predictions
 */
export const resolveInterventionLoop = onDocumentUpdated(
  {
  ...
  async (event) => {
    ...
    const beforeResults = before.practiceTestResults || {};
    const afterResults = after.practiceTestResults || {};

    // Detect new test: find a testId in after that has more attempts than before
    let newTestId: string | null = null;
    let newAttempt: Record<string, unknown> | null = null;

    for (const [testId, testData] of Object.entries(afterResults)) {
      const afterAttempts = (testData as Record<string, unknown>)?.attempts as unknown[];
      const beforeAttempts = (beforeResults[testId] as Record<string, unknown>)?.attempts as unknown[] || [];

      if (afterAttempts && afterAttempts.length > beforeAttempts.length) {
        newTestId = testId;
        newAttempt = afterAttempts[afterAttempts.length - 1] as Record<string, unknown>;
        break;
      }
    }

    // No new test detected — this is either our own write-back
    // (fingerprint/prediction updates) or an unrelated field change
    if (!newTestId || !newAttempt) return;

    const diagnosticData = newAttempt.diagnosticData as Record<string, unknown>;
    if (!diagnosticData) return; // No diagnostic data

    const userId = event.params.userId;
    logger.info(`[resolveInterventionLoop] New test detected for ${userId}: ${newTestId}`);
    ...
    try {
      ...
      // Current state
      const interventionLog = (after.interventionLog || []) as Array<Record<string, unknown>>;
      const skillProgress = (after.skillProgress || {}) as Record<string, Record<string, unknown>>;
      const predictionLog = (after.predictionLog || []) as Array<Record<string, unknown>>;

      // ─── 1. Resolve unresolved interventions ───
      const skillAnalysis = diagnosticData.skillAnalysis as Record<string, unknown>;
      const weakSkills = (skillAnalysis?.weakSkills || []) as Array<Record<string, unknown>>;
      const strongSkills = (skillAnalysis?.strongSkills || []) as Array<Record<string, unknown>>;
      const allTestSkills = [...weakSkills, ...strongSkills];

      // Build set of skills tested
      const testedSkillIds = new Set<string>();
      for (const s of allTestSkills) {
        if (s.skillId) testedSkillIds.add(s.skillId as string);
      }

      // Build current mastery map from skillProgress
      const currentMastery: Record<string, number> = {};
      for (const [skillId, data] of Object.entries(skillProgress)) {
        currentMastery[skillId] = (data.mastery as number) ?? 0;
      }

      let interventionsUpdated = false;
      for (let i = 0; i < interventionLog.length; i++) {
        const intervention = interventionLog[i];
        if (intervention.resolved) continue;

        const skillIds = (intervention.skillIds || []) as string[];
        const coveredSkills = skillIds.filter((sid: string) => testedSkillIds.has(sid));

        if (coveredSkills.length === 0) continue;

        // Compute post-metrics
        const postMastery: Record<string, number> = {};
        const delta: Record<string, number> = {};
        const preMastery = (intervention.preMetrics as Record<string, unknown>)?.skillMastery as Record<string, number> || {};

        let totalDelta = 0;
        for (const skillId of coveredSkills) {
          postMastery[skillId] = currentMastery[skillId] ?? 0;
          delta[skillId] = postMastery[skillId] - (preMastery[skillId] ?? 0);
          totalDelta += delta[skillId];
        }

        const avgDelta = coveredSkills.length > 0 ? totalDelta / coveredSkills.length : 0;

        // Get current error types
        const errorCounts = (diagnosticData.errorPatterns as Record<string, unknown>)?.counts as Record<string, number> || {};
      ...
      // ─── 2. Update student fingerprint ───
      const fingerprint = computeFingerprintFromDiagnostic(diagnosticData, skillProgress);
      const existingSnapshots = (after.studentFingerprint as Record<string, unknown>)?.snapshots as Array<Record<string, unknown>> || [];

      const snapshot = {
        testId: newTestId,
        timestamp: new Date().toISOString(),
        dimensions: {
          speedProfile: fingerprint.speedProfile,
          conceptualDepth: fingerprint.conceptualDepth,
          trapResistance: fingerprint.trapResistance,
          staminaProfile: fingerprint.staminaProfile,
          confidenceCalibration: fingerprint.confidenceCalibration,
          learningVelocity: fingerprint.learningVelocity,
        },
        scaledScore: (diagnosticData.score as Record<string, unknown>)?.scaled ?? null,
      };

      existingSnapshots.push(snapshot);
      while (existingSnapshots.length > 10) existingSnapshots.shift();
      ...
      for (let i = 0; i < predictionLog.length; i++) {
        if (predictionLog[i].resolved) continue;

        const preds = predictionLog[i].predictions as Record<string, unknown>;
        const actualScore = (diagnosticData.score as Record<string, unknown>)?.scaled as number;

        const expectedRange = preds?.expectedScoreRange as Record<string, number>;
        const scoreInRange = actualScore != null && expectedRange &&
          actualScore >= expectedRange.low && actualScore <= expectedRange.high;

        const actualWeak = new Set(weakSkills.map((s) => s.skillId as string));
        const likelyStruggle = (preds?.likelyStruggleSkills || []) as Array<Record<string, unknown>>;
        const struggleSkillsHit = likelyStruggle.filter(
          (s) => actualWeak.has(s.skillId as string)
        ).length;

        const trapVulns = (preds?.trapVulnerabilities || []) as Array<Record<string, unknown>>;
        const errorCounts = (diagnosticData.errorPatterns as Record<string, unknown>)?.counts as Record<string, number> || {};
        const trapPredictionHit = trapVulns.filter(
          (t) => (errorCounts[t.trapType as string] || 0) >= 2
        ).length;

        const actualArchetype = (diagnosticData.mistakeFingerprint as Record<string, unknown>)?.archetype;
      ...
      const recentScores = getRecentScoresFromResults(afterResults, 3);
      const expectedScoreRange = predictScoreRangeServer(recentScores, updatedFingerprint);

      const likelyStruggleSkills: Array<Record<string, unknown>> = [];
      for (const [skillId, data] of Object.entries(skillProgress)) {
        const mastery = (data.mastery as number) ?? 100;
        if (mastery >= 50 || (data.attempts as number) < 2) continue;

        const hasNegativeDelta = interventionLog.some(
          (iv) => !!(iv.resolved) &&
            (iv.skillIds as string[])?.includes(skillId) &&
            ((iv.postMetrics as Record<string, Record<string, number>>)?.delta?.[skillId] ?? 0) < 0
        );

        likelyStruggleSkills.push({
          skillId,
          reason: `Mastery at ${mastery}%${hasNegativeDelta ? ", previous intervention showed no improvement" : ""}`,
          confidence: mastery < 30 || hasNegativeDelta ? "high" : "medium",
          _weight: (100 - mastery) * (hasNegativeDelta ? 1.5 : 1),
        });
      }

      likelyStruggleSkills.sort((a, b) => (b._weight as number) - (a._weight as number));
      const top3Struggles = likelyStruggleSkills.slice(0, 3).map(({_weight, ...rest}) => rest);

      const newPrediction = {
        id: `pred_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 7)}`,
        createdAt: new Date().toISOString(),
        createdAfterTestId: newTestId,
        predictions: {
          expectedScoreRange,
          likelyStruggleSkills: top3Struggles,
          trapVulnerabilities: [],
          expectedArchetype: updatedFingerprint.archetype || "balanced",
        },
        resolved: false,
        actualTestId: null,
        accuracy: null,
      ...
      predictionLog.push(newPrediction);
      while (predictionLog.length > 10) predictionLog.shift();

      // ─── 5. Write all updates in a single batch ───
      const updates: Record<string, unknown> = {
        studentFingerprint: updatedFingerprint,
        predictionLog,
      };

      if (interventionsUpdated) {
        updates.interventionLog = interventionLog;
      }

      batch.update(progressRef, updates);
      await batch.commit();

      logger.info(`[resolveInterventionLoop] Loop closed for ${userId}: ` +
        `${interventionsUpdated ? "interventions resolved" : "no interventions to resolve"}, ` +
        `fingerprint updated, ` +
        `${predictionsUpdated ? "predictions validated" : "no predictions to validate"}, ` +
        `new prediction generated`);
    } catch (error) {
      logger.error("[resolveInterventionLoop] Error:", error);
    }
  }
);

// ─── Helper functions for resolveInterventionLoop ───

function computeFingerprintFromDiagnostic(
  diagnosticData: Record<string, unknown>,
  skillProgress: Record<string, Record<string, unknown>>
) {
  const ta = diagnosticData.timeAnalysis as Record<string, unknown>;
  const avgTime = (ta?.avgTimePerQuestion as number) || 60;
  const accuracy = ((diagnosticData.score as Record<string, unknown>)?.percentCorrect as number) ?? 50;

  const rawSpeed = Math.round(((120 - avgTime) / 90) * 100);
  const accuracyWeight = accuracy / 100;
  const speedProfile = clampVal(Math.round(rawSpeed * accuracyWeight + 50 * (1 - accuracyWeight)), 0, 100);

  const ep = diagnosticData.errorPatterns as Record<string, unknown>;
  const summary = (ep?.summary || []) as Array<Record<string, unknown>>;
  const totalWrong = (ep?.totalWrong as number) || 1;

  const conceptualGaps = summary.find((e) => e.type === "CONCEPTUAL_GAP");
  const conceptualPct = conceptualGaps ? ((conceptualGaps.count as number) / totalWrong) * 100 : 0;
  const conceptualDepth = clampVal(Math.round(100 - (conceptualPct * 1.5)), 0, 100);

  const traps = summary.find((e) => e.type === "TRAP_SUSCEPTIBILITY");
  const trapPct = traps ? ((traps.count as number) / totalWrong) * 100 : 0;
  const trapResistance = clampVal(Math.round(100 - (trapPct * 2)), 0, 100);

  const stamina = diagnosticData.stamina as Record<string, unknown>;
  const staminaProfile = stamina?.staminaScore != null ? clampVal(Math.round(stamina.staminaScore as number), 0, 100) : 50;

  const lv = diagnosticData.learningVelocity as Record<string, unknown>;
  let learningVelocity = 50;
  if (lv?.velocity) {
    const v = lv.velocity as number;
    if (v >= 5) learningVelocity = 100;
    else if (v >= 0) learningVelocity = clampVal(Math.round(50 + (v / 5) * 50), 50, 100);
    else learningVelocity = clampVal(Math.round(50 + (v / 2) * 50), 0, 50);
  }

  let confidenceCalibration = 50;
  const ap = diagnosticData.answerPatterns as Record<string, unknown>;
  const ac = ap?.answerChanges as Record<string, unknown>;
  if (ac && (ac.total as number) > 0) {
    const changeAccuracy = (ac.changedToCorrect as number) / (ac.total as number);
    confidenceCalibration += (changeAccuracy - 0.5) * 40;
  }
  confidenceCalibration = clampVal(Math.round(confidenceCalibration), 0, 100);

  const fp = diagnosticData.mistakeFingerprint as Record<string, unknown>;
  const archetype = (fp?.archetype as string) || "balanced";

  const counts = (ep?.counts || {}) as Record<string, number>;
  ...
  const domains = (diagnosticData.domainAnalysis || []) as Array<Record<string, unknown>>;
  let strongestDomain = "N/A";
  let weakestDomain = "N/A";
  if (domains.length > 0) {
    let sAcc = -1;
    let wAcc = 101;
    for (const d of domains) {
      const acc = (d.accuracy as number) ?? 0;
      if (acc > sAcc) { sAcc = acc; strongestDomain = (d.displayName || d.domain || "N/A") as string; }
      if (acc < wAcc) { wAcc = acc; weakestDomain = (d.displayName || d.domain || "N/A") as string; }
    }
  }
...
function getRecentScoresFromResults(
  practiceTestResults: Record<string, Record<string, unknown>>,
  n: number
): number[] {
  const allAttempts: Array<{score: number; date: string}> = [];
  for (const testData of Object.values(practiceTestResults)) {
    const attempts = (testData?.attempts || []) as Array<Record<string, unknown>>;
    for (const attempt of attempts) {
      if (attempt.scaledScore) {
        allAttempts.push({
          score: attempt.scaledScore as number,
          date: attempt.completedAt as string,
        });
      }
    }
  }
...
function predictScoreRangeServer(
  recentScores: number[],
  fingerprint: Record<string, unknown>
): {low: number; high: number} {
  if (recentScores.length === 0) return {low: 400, high: 800};

  const avg = recentScores.reduce((a, b) => a + b, 0) / recentScores.length;
  const velocityFactor = (fingerprint?.learningVelocity as number) ?? 50;
