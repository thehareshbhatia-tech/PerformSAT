...
- **[BUG/non-design] Prediction-card shows a contradictory verdict** ("You scored 530 — outside range" while the band reads 490–610, 530 is inside). Root-caused 2026-06-08 (live-confirmed on the owner account, `accuracy.scoreInRange === false`, NOT null): `PredictedVsActualCard` prints a re-derived *current* `actualScore` (`predictionSummary.js:78` → `pickActualScoreForTest`, section scale = 530) next to a verdict (`accuracy.scoreInRange`) that `predictionEngine.validatePrediction` computed at validation time against a *different* headline score (likely composite/cross-scale, genuinely outside the band). Display and verdict are about two different numbers → the contradiction. Proper fix needs a data-shape decision: persist the validation-time actual score on the prediction record and display THAT (so the shown number matches the verdict), or suppress the within/outside-range clause when the displayed score wasn't the one validated. While there, also fix the related latent bug: `predictionSummary.js:88` coerces the tri-state `scoreInRange` `null` ("couldn't compare" / cross-scale) to `false`, so a genuine couldn't-compare prediction would also wrongly read "outside range" (card `PredictedVsActualCard.jsx:45` prints "outside range" for any falsy value). A scoped fix + regression test was prototyped and reverted (it correctly preserved `null` but did not resolve this `false`-verdict display mismatch). Use `/investigate`.
