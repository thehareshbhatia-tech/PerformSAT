...
The product was renamed PerformSAT → SEVA on 2026-05-21 (briefly via Sura → Seva). The logo (2026-06-15) is a lowercase **"seva"** wordmark in rounded Baloo 2 next to a tri-color **"S" mark** (orange/purple/lime horizontal bands). Both live in `src/components/ui/{Mark,Wordmark}.jsx` — the mark is a Baloo 2 "S" with a vertical gradient clipped to the glyph via `background-clip: text`; the favicon/PWA/OG PNGs in `public/` are rendered from that mark. In prose the brand name is still written **SEVA**. Internal identifiers (the `performsat:` log-scope prefix, `localStorage['performsat:logVerbose']`, the `PERFORMSAT_TEST_EMAIL`/`PERFORMSAT_TEST_PASSWORD` env vars, the `PerformSAT` React component name in `src/App.jsx`, the repo directory `~/PerformSAT/`) deliberately retain the old name to avoid coordinated-rename risk. New code may use either name in identifiers — be consistent within a module.
...
|------------|-------|-------|
| Question banks (math + R&W) | `src/data/questions/bank/index.js` (math, **1373 hand-authored items** after the 2026-05-18 PT-coverage batch v3.4) and `src/data/questions/rwBank/index.js` (R&W, 648 items, flattened from 12 test bundles) | Both production. Section-tag weakness contract dispatches by `weakness.section`. CB-skill coverage: **19 healthy / 0 thin / 0 empty**. Surfaced patterns (≥4 items): **121** (was 102 at start of 2026-05-18; +19 new canonicals incl. median, mode, triangle-area, sector-area, special-right triangles, radians/degrees, vertical angles, polynomial remainder theorem, quadratic inequality, radical equation, exponential common-base, completing the square, distance between x-intercepts, simple interest, D=RT, average rate, scaling a data set). |
