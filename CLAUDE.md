...
|------------|-------|-------|
| Question banks (math + R&W) | `src/data/questions/bank/index.js` (math, **1125 hand-authored items** after the 2026-05-16 CB gap-closure batch; 1083 surface as drillable after the audit's MC-filter) and `src/data/questions/rwBank/index.js` (R&W, 648 items, flattened from 12 test bundles) | Both production. Section-tag weakness contract dispatches by `weakness.section`. CB-skill coverage: **10 healthy / 9 thin / 0 empty** (was 8/10/1 before). |
